...
router.get('/permisos', authenticate, requirePermission('config.sistema'),
...
router.get('/permisos/rol/:id', authenticate, requirePermission('config.sistema'),
...
router.post('/permisos/rol/:id', authenticate, requirePermission('config.sistema'),
  async (req: Request, res: Response, next: NextFunction) => {
    try {
      const { id_permiso } = req.body;
      if (!id_permiso) { res.status(400).json({ error: 'id_permiso requerido' }); return; }
      const data = await configuracionService.asignarPermiso(Number(req.params.id), Number(id_permiso));
      res.status(201).json(successResponse(data, 'Permiso asignado'));
    } catch (e) { next(e); }
  }
);

router.put('/permisos/rol/:id/sync', authenticate, requirePermission('config.sistema'),
  async (req: Request, res: Response, next: NextFunction) => {
    try {
      const { ids_permisos } = req.body;
      if (!Array.isArray(ids_permisos)) { res.status(400).json({ error: 'ids_permisos debe ser un array' }); return; }
      const data = await configuracionService.sincronizarPermisos(Number(req.params.id), ids_permisos);
      res.json(successResponse(data, 'Permisos sincronizados'));
    } catch (e) { next(e); }
  }
);

router.delete('/permisos/rol/:id/:permiso', authenticate, requirePermission('config.sistema'),
  async (req: Request, res: Response, next: NextFunction) => {
    try {
      await configuracionService.revocarPermiso(Number(req.params.id), Number(req.params.permiso));
