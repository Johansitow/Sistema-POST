...
import {
  ...
  Send, Truck, Ban, Hash, ArrowRight,
} from 'lucide-react';
...
// HELPERS
// ============================================================================
const ESTADO_CFG: Record<EstadoListaCompras, { label: string; cls: string; icon: React.ReactNode }> = {
...
const EstadoBadge: React.FC<{ estado: EstadoListaCompras }> = ({ estado }) => {
  const cfg = ESTADO_CFG[estado] ?? ESTADO_CFG.generada;
  return (
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-50 via-indigo-50/20 to-slate-100">
      ...
      <div className="max-w-7xl mx-auto px-6 py-6 space-y-5">
        ...
        <div className="bg-white rounded-2xl border border-slate-100 shadow-sm p-5 flex items-start gap-4">
          ...
          <div>
            <p className="font-semibold text-slate-700 text-sm">Generación automática activa</p>
            <p className="text-slate-500 text-sm mt-1">
              El sistema verifica el stock cada hora y crea automáticamente una lista de compras cuando
              algún producto llega a su nivel mínimo. El mejor proveedor disponible se asigna
              automáticamente a cada producto.
            </p>
          </div>
        </div>
      </div>
